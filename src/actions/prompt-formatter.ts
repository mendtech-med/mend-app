// server/actions/promptFormatter.ts
...
export const promptFormater = (prompt: string) => {
    return `\n\nHuman:\n  ${prompt}\n\nAssistant:\n`
}

export const formatFindAnalyticsPrompt = ({
    title,
    selection,
    targetAudience,
    targetAudienceLevel,
    ...
    return `
        Task: Rewrite the following SelectedText by enriching it with more ${type}.

        SelectedText: ${selection}
        EnrichmentType: ${type}

        <instructions>
            You're a news generator assistant. Your Task is to rewrite the 'SelectedText' by enriching it with more ${type} from your knowledge.
            You strictly need to follow the below instructions while rewriting the 'SelectedText':
            Do everything step by step. This can be complicated.

            The most important thing - Rewrite the 'SelectedText' using the ${type} you find. Try to keep the length of the generated text similar to 'SelectedText'. Here are your step by step instructions -
            1. The targeted audience will be from the ${targetAudience} industry.
            2. There can be multiple levels of audience. Example:
                * 'Entry Level'
                * 'Mid Level'

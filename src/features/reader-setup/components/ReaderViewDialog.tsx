...
interface ReaderViewDialogProps {
  reader: IContentClassification;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export const ReaderViewDialog = ({
  reader,
  open,
  onOpenChange,
}: ReaderViewDialogProps) => {
  return (
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-gray-600 capitalize">{reader.categoryType}</p>
            </div>

            <div>
              <h3 className="font-medium mb-2">Preferences</h3>
              {Object.entries(reader.preferences).map(([group, items]) => (

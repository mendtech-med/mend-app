...
const DiscussPage = () => {
  ...
  // Sample prompts to display when chat is empty
  const samplePrompts = [
    'What are the latest financial reports?',
    'Summarize the recent market trends',
    'What are the top news from last week?',
    'Tell me about upcoming company events',
    'How is our quarterly performance?',
  ];
  ...
  const sendMessage = async () => {
    ...
    const userMessage = new Message(messageText, 'sent', USER_AVATAR);
    ...
    try {
      ...
      // This is just a placeholder - implement actual file upload logic
      if (selectedFile) {
        // Example: You might want to upload the file to a server
        console.log('File would be uploaded:', selectedFile);
        // const formData = new FormData();
        // formData.append('file', selectedFile);
        // await axiosInstanceChatbot.post('/upload', formData);
      }
  ...
  const handleReaction = (messageId: string, reaction: 'like' | 'dislike') => {
    setMessages((prevMessages) =>
      prevMessages.map((message) => {
        if (message.id === messageId && message.type === 'received') {
          return {
            ...message,
            liked: reaction === 'like' ? !message.liked : false,
            disliked: reaction === 'dislike' ? !message.disliked : false,
          };
        }
        return message;
      })
  ...
  return (
    <div className="chat-container">
            ...
            <DropdownMenu.Trigger>
              <button className="btn bg-theme-main/5 p-2 rounded-xl flex items-center content-between gap-2 truncate text-theme-main">
                <div
                  ...
                  className="tooltip truncate w-40"
                >
                  {selectedNews?.newsTitle || 'Select chat context'}
                </div>
      ...
      {/* Chat messages with sample prompts when empty */}
      <div className="chat-messages" ref={chatMessagesRef}>
            ...
            <div className="grid grid-cols-1 gap-2 w-full max-w-md">
              {samplePrompts.map((prompt, index) => (
                <button
                  ...
                  className="text-left p-3 border border-gray-200 rounded-lg hover:bg-theme-main/5 transition-colors"
                >
                  {prompt}
                </button>
              ))}
                ...
                {message.type === 'received' && (
                  <div className="absolute bottom-2 right-2 flex space-x-2">
                    ...
                    <button
                      ...
                      className={`p-1 hover:bg-gray-100 rounded-full transition-colors ${message.liked ? 'text-blue-500' : ''}`}
                      title="Like"
                    >
                    ...
                    <button
                      ...
                      className={`p-1 hover:bg-gray-100 rounded-full transition-colors ${message.disliked ? 'text-red-500' : ''}`}
                      title="Dislike"
                    >
        ...
        {isTyping && (
          <div className="message received">
            <div className="bg-theme-main/5 p-2 rounded-full">AI</div>
